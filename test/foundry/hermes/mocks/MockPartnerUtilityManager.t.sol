...
import { IPartnerUtilityManager, PartnerUtilityManager, IUtilityManager, UtilityManager } from "@maia/PartnerUtilityManager.sol";
...
contract MockPartnerUtilityManager is PartnerUtilityManager {
    ...
    modifier checkPartnerGovernance(uint256 amount) override {
        if (
            userClaimablePartnerGovernance[msg.sender] <
            amount + userClaimedPartnerGovernance[msg.sender]
        ) revert InsufficientShares();
        _;
    }
}
