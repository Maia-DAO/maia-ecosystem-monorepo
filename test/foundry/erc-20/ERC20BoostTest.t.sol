...
import { MockERC20Gauges, IERC20Gauges, ERC20Gauges, ERC20 } from "./mocks/MockERC20Gauges.t.sol";

import { MockERC20Boost, IERC20Boost, ERC20Boost } from "./mocks/MockERC20Boost.t.sol";
...
// import {FlywheelGaugeRewards, ERC20, Authority} from "../mocks/MockBaseV2Gauge.sol";
...
contract ERC20BoostTest is DSTestPlus {
    ...
    function setUp() public {
        ...
        hevm.mockCall(
            ...
            abi.encode(ERC20(address(0xDEAD)))
        );
        hevm.mockCall(
            address(0),
            abi.encodeWithSignature("gaugeToken()"),
            abi.encode(ERC20Gauges(address(0xBEEF)))
        );
        hevm.mockCall(
            address(this),
        ...
        gauge1 = address(
            new MockBaseV2Gauge(FlywheelGaugeRewards(address(0)), address(0), address(0))
        );
        gauge2 = address(
            new MockBaseV2Gauge(FlywheelGaugeRewards(address(0)), address(0), address(0))
        );
    ...
    // function testDecrementGaugesAllBoostIndexed() public {
    //     testAttachTwoGauges();
    //     hevm.prank(address(1));
    //     token.decrementGaugesAllBoostIndexed(1, 1);
    //     (uint128 newUserBoost,) = token.getUserGaugeBoost(address(1), gauge1);
    //     (uint128 newUserBoost2,) = token.getUserGaugeBoost(address(1), gauge2);
    //     console2.log(newUserBoost, newUserBoost2);
    //     require(newUserBoost == 100 ether, "userBoost not updated");
    //     require(newUserBoost2 == 0 ether, "userBoost not updated");
    //     require(token.getUserBoost(address(1)) == 0 ether, "total userBoost doesn't match");
    // }
