...
contract BranchBridgeAgent is IBranchBridgeAgent, BridgeAgentConstants {
    ...
    constructor(
        ...
        require(_lzEndpointAddress != address(0), "Layerzero Endpoint Address cannot be the zero address.");
        ...
        requiresEndpoint(_endpoint, _srcAddress)
    {
        ...
        if (flag == 0x00) {
            ...
            if (executionState[nonce] != 0) revert AlreadyExecutedTransaction();
            ...
            if (executionState[nonce] == 1) {
                revert AlreadyExecutedTransaction();
            } else {
                //Set settlement to retrieve mode, if not already set.
                if (executionState[nonce] == 0) executionState[nonce] = 2;
    ...
    function _execute(bool _hasFallbackToggled, uint256 _settlementNonce, address _refundee, bytes memory _payload)
        private
    {
        //Update tx state as executed
        executionState[_settlementNonce] = 1;
        ...
        if (!success) {
            ...
            if (_hasFallbackToggled) {
                ...
                executionState[_settlementNonce] = 2;

                // Perform fallback call
                _performFallbackCall(payable(_refundee), _payload);
            } else {
     ...
     *   @param _payload params for root bridge agent execution.
     */
    function _performFallbackCall(address payable _refundee, bytes memory _payload) internal virtual {
        //Sends message to LayerZero messaging layer
        ILayerZeroEndpoint(lzEndpointAddress).send{value: address(this).balance}(
            rootChainId, rootBridgeAgentPath, abi.encodePacked(bytes1(0x09), _payload), _refundee, address(0), ""
        );
    }
    ...
    function _fallback(bytes calldata _payload) internal {
        ...
        if (flag < 0x03) {
            ...
            // Save nonce
            _depositNonce = uint32(bytes4(_payload[PARAMS_START_SIGNED:PARAMS_TKN_START_SIGNED]));

            /// DEPOSIT FLAG: 6
        } else if (flag == 0x06) {
            // Save nonce
            _depositNonce =
                uint32(bytes4(_payload[PARAMS_START_SIGNED + PARAMS_START:PARAMS_START_SIGNED + PARAMS_TKN_START]));
        ...
        getDeposit[_depositNonce].status = STATUS_FAILED;

        emit LogFallback(_depositNonce);
    }
