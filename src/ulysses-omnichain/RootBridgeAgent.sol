...
import {BridgeAgentConstants} from "./BridgeAgentConstants.sol";
import {
    GasParams,
    DepositParams,
...
contract RootBridgeAgent is IRootBridgeAgent, BridgeAgentConstants {
    ...
    function retrieveSettlement(uint32 _settlementNonce, GasParams calldata _gParams) external payable lock {
        ...
        //Retrieve Deposit
        _performCall(settlementReference.dstChainId, payable(msg.sender), payload, _gParams);
    }
    ...
    function redeemSettlement(uint32 _settlementNonce) external override lock {
        ...
        if (settlement.status != STATUS_FAILED || settlementOwner == address(0)) {
    ...
    function lzReceiveNonBlocking(
        ...
        if (_payload[0] == 0x00) {
            ...
            if (executionState[_srcChainId][nonce] != 0) {
            ...
            if (executionState[_srcChainId][nonce] == 1) {
                revert AlreadyExecutedTransaction();
            } else {
                //Set settlement to retrieve mode, if not already set.
                if (executionState[_srcChainId][nonce] == 0) executionState[_srcChainId][nonce] = 2;
     ...
     *   @param _payload Payload of message to be executed by the Root Bridge Agent Executor Contract.
    ...
    function _execute(
        ...
        executionState[_srcChainId][_depositNonce] = 1;

        //Try to execute the remote request
        (bool success,) = bridgeAgentExecutorAddress.call{value: address(this).balance}(_payload);
        ...
        if (!success) {
            ...
            if (_hasFallbackToggled) {
                ...
                executionState[_srcChainId][_depositNonce] = 2;
                //TODO: _performFallbackCall(_payload, _srcChainId); ensure there is always a valid user controlled refundee
                _performFallbackCall(payable(_refundee), _payload, _srcChainId);
            } else {
                // If no fallback is requested revert allowing for retry.
     ...
     *   @param _dstChainId Chain ID of destination chain.
     */
    function _performFallbackCall(address payable _refundee, bytes memory _payload, uint16 _dstChainId) internal {
        //Sends message to LayerZero messaging layer
        ILayerZeroEndpoint(lzEndpointAddress).send{value: address(this).balance}(
            _dstChainId,
            getBranchBridgeAgentPath[_dstChainId],
            abi.encodePacked(bytes1(0x04), _payload),
            payable(_refundee),
            address(0),
            ""
        );
     ...
     *   @param _srcChainId Layer Zero Chain ID of the chain from where fallback was initiated.
    ...
    function _fallback(uint16 _srcChainId, bytes calldata _payload) internal {
        ...
        if (flag == 0x80) {
            _settlementNonce = uint32(bytes4(_payload[PARAMS_START_SIGNED:PARAMS_TKN_START_SIGNED]));
            ...
            /// SETTLEMENT FLAG: Unknown
        } else {
            revert UnknownFlag();
        }
        /// Reopen Settlement for redemption
        getSettlement[_settlementNonce].status = STATUS_FAILED;

        emit LogFallback(_settlementNonce, _srcChainId);
    }
