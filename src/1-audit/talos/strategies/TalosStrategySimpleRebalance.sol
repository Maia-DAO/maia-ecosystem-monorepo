...
abstract contract TalosStrategySimpleRebalance is TalosBaseStrategy {
    ...
    function doRerange() internal override returns (uint256 amount0, uint256 amount1) {
        (tickLower, tickUpper, amount0, amount1, tokenId, liquidity) = nonfungiblePositionManager
            .rerange(
    ...
    function doRebalance() internal override returns (uint256 amount0, uint256 amount1) {
        ...
        PoolActions.ActionParams memory actionParams = PoolActions.ActionParams(
            pool,
            optimizer,
            token0,
            token1,
